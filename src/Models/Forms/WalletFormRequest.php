...
class WalletFormRequest extends FormRequest
{
    ...
    {
        $rules = [
            'setting_id' => ['required','string','exists:'.config('wk-core.table.point.settings').',id'],
        ...
        if (
            ...
            $rules = array_merge($rules, ['id' => ['required','string','exists:'.config('wk-core.table.point.wallets').',id']]);
        }
